    ...
    with sqlite3.connect(database) as dbconn:
        ...
        for _id, grouping in itertools.groupby(result, key=lambda r: r[0]):
            ...
            if content is not None:
                records[content] = tags
        return [Record(content, tags) for content, tags in records.items()]
...
def filter_data(records: list[Record], is_admin=False, exclusion_filters: list[str] = None):
    if exclusion_filters is None:
        exclusion_filters = []
    filters = exclusion_filters.copy()
    if not is_admin:
        filters.append('private')
    for record in records:
        if any(f.found(record) for f in build_filters(*filters)):
            continue
        yield record
...
class TestApplication(unittest.TestCase):
    ...
    def test_4_toggle_admin(self):
        for is_admin in (True, False, True):
            ...
            if is_admin:
                # Only check if there is at least one record, and if so, that at least one has 'private'
                if data:
                    assert any('private' in r.tags for r in data)
            else:
                # Only check if there is at least one record, and if so, that at least one does not have 'private'
                if data:
                    assert any('private' not in r.tags for r in data)
