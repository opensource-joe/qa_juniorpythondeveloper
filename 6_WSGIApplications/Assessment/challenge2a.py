...
###############################################################################
# Create middleware to wrap the above app function.
# 1. The middleware must parse the query string and extract the optional parameter named: theme.
# 2. Assign the theme to the middleware's environ dictionary with a key named: THEME.
# 3. Assign your middleware to the theme_middleware name binding below.
def theme_middleware(app):
    def middleware(environ, start_response):
        # Parse the query string from the environ dictionary.
        query_string = environ.get('QUERY_STRING', '')
        params = parse_qs(query_string)

        # Extract the theme parameter, defaulting to None if not provided.
        theme = params.get('theme', [None])[0]

        # Assign the theme to the environ dictionary.
        environ['THEME'] = theme if theme else 'default'

        # Call the original app with the modified environ.
...
theme_middleware = theme_middleware
