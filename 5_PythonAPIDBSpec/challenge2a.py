...
def add_user(dbconn: sqlite3.Connection, full_name: str, reference: str, postcount: int):
    ...
    '''
    dbconn.execute(
        "INSERT INTO user (full_name, reference, postcount) VALUES (?, ?, ?)",
        (full_name, reference, postcount)
    )
    dbconn.commit()
...
def top_poster_reference(dbconn: sqlite3.Connection) -> str:
    ...
    '''
    cursor = dbconn.execute("SELECT reference FROM user ORDER BY postcount DESC LIMIT 1")
    row = cursor.fetchone()
    return row[0] if row else None

# Complete the following function.
def post_count_gt_n(dbconn: sqlite3.Connection, n: int) -> list[tuple[str, int]]:
    ...
    '''
    cursor = dbconn.execute("SELECT reference, postcount FROM user WHERE postcount > ? ORDER BY postcount DESC", (n,))
