...
class Power:
    ...
    def activate(self, amplifier_callable: callable = randint):
        ...
        avatar_str = ' '.join([self.avatar] * amplifier)
        points_total = self.points * amplifier
        return avatar_str, points_total
...
class SuperSlothBot:
    randint_gen_callable = staticmethod(randint)
    ...
    def activate(self, number: int):
        ...
        self.last_result = self.last_power.activate(SuperSlothBot.randint_gen_callable)
        return self.last_result
    
    def __str__(self):
        if self.last_result:
            avatar, points = self.last_result
        else:
            avatar, points = '', 0
        return f'{self.name} counjours: {avatar} for a total of: {points} points.'
...
def play(rounds=5):
    ...
    for round in range(rounds):
        ...
        for n, bot in bots.items():
            bot.activate(SuperSlothBot.randint_gen_callable(1, 50))
            scores[n] += bot.last_result[1]
